...
        // -----------------------------
        // 8. Inject Gmail Credentials into ECS Booking Service (FIXED)
        // -----------------------------
...
                        --query "taskDefinition" > task_def.json
                        ...
                        // Use a separate PowerShell call for clarity and error handling
                    powershell -Command "
                        \$json = Get-Content 'task_def.json' | ConvertFrom-Json;
                        \$envList = @();

                        // Filter out old credentials (EMAIL_USER and EMAIL_PASS)
...
                        }
                        ...
                        // Add new credentials (using the Jenkins credentials variables)
...
                    aws ecs describe-task-definition --task-definition %TASK_DEF_ARN% --query "taskDefinition" > task_def_crowdpulse.json
                        ...
                        // Remove existing YOUTUBE_API_KEY if present
                    powershell -Command "
                        \$json = Get-Content 'task_def_crowdpulse.json' | ConvertFrom-Json; 
                        \$envList = @();
                        foreach (\$env in \$json.containerDefinitions[0].environment) {
                            if (\$env.name -ne 'YOUTUBE_API_KEY') {
                                \$envList += \$env;
                            }
                        }
...
            }
        }
    } // <-- This closing brace was missing or misplaced in your original file
